...
test.describe("Sin sesión", () => {
  test("las pantallas de dentro mandan al login", async ({ page }) => {
    for (const ruta of ["/empresas", "/admin/usuarios", "/admin/auditoria", `/e/${EMPRESA_A}`]) {
      await irA(page, ruta)
      await expect(page, `${ruta} no exigió sesión`).toHaveURL(/\/login/)
    }
  })

  test("una cuenta suspendida no entra", async ({ page }) => {
  ...
  test("una contraseña incorrecta no entra", async ({ page }) => {
    await irA(page, "/login")
...
test.describe("Super admin", () => {
  test("ve la administración de la plataforma", async ({ superAdmin }) => {
  ...
  test("entra a usuarios y auditoría", async ({ superAdmin }) => {
  ...
  test("ve las dos empresas de prueba y puede crear", async ({ superAdmin }) => {
    await irA(superAdmin, "/empresas")
...
test.describe("Coordinador", () => {
  test("no ve la administración de la plataforma", async ({ coordinador }) => {
  ...
  test("la auditoría le queda cerrada aunque entre por la URL", async ({ coordinador }) => {
  ...
  test("administra su empresa: sedes, usuarios y configuración", async ({ coordinador }) => {
...
test.describe("Asesor", () => {
  test("no ve administración ni de la plataforma ni de la empresa", async ({ asesorA }) => {
  ...
  test("sí ve los módulos de captura de su empresa", async ({ asesorA }) => {
    await irA(asesorA, `/e/${EMPRESA_A}`)
  ...
  test("las empresas reales del cliente tampoco", async ({ asesorA }) => {
  ...
  test("en su lista solo aparece su empresa", async ({ asesorA }) => {
  ...
  test("la administración de la plataforma no le abre", async ({ asesorA }) => {
    await irA(asesorA, "/admin/usuarios")
    // Ni la lista de las 50 personas reales ni el botón de invitar.
    await expect(asesorA.getByRole("button", { name: /Invitar|Nuevo usuario/i })).toHaveCount(0)
  })
})
