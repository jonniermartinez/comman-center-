...
test.describe("Sin sesión", () => {
  ...
  test(
    ...
    anotar({
      ...
      tipo: "seguridad",
      porque:
        "Suspender tiene que cortar el login, no solo esconder los datos.",
    }),
    async ({ page }) => {
      ...
      await expect(page.locator("body")).toContainText(
        /no|error|incorrect|bloque|suspend/i,
      );
    },
  );
...
test.describe("Super admin", () => {
  test(
    ...
    anotar({
      ...
      tipo: "feature",
      porque:
        "El super admin es el único que administra usuarios y consulta el log.",
    }),
    async ({ superAdmin }) => {
      await irA(superAdmin, "/empresas");
      await expect(
        superAdmin.getByRole("link", { name: "Usuarios" }),
      ).toBeVisible();
      await expect(
        superAdmin.getByRole("link", { name: "Auditoría" }),
      ).toBeVisible();
    },
  );
  ...
  test(
    ...
    anotar({
      ...
      tipo: "feature",
      porque:
        "Las dos pantallas de plataforma tienen que abrir para quien sí manda.",
    }),
    async ({ superAdmin }) => {
  ...
  test(
    ...
    async ({ superAdmin }) => {
      await irA(superAdmin, "/empresas");
      await expect(
        superAdmin.getByRole("link", { name: /Nueva empresa/ }),
      ).toBeVisible();
...
test.describe("Coordinador", () => {
  test(
    ...
    async ({ coordinador }) => {
      ...
      await expect(
        coordinador.getByRole("link", { name: "Auditoría" }),
      ).toHaveCount(0);
      await expect(
        coordinador.getByRole("link", { name: /Nueva empresa/ }),
      ).toHaveCount(0);
    },
  );
  ...
  test(
    ...
    anotar({
      ...
      tipo: "seguridad",
      porque:
        "Que no salga en el menú no basta: la dirección se puede escribir.",
    }),
    async ({ coordinador }) => {
      ...
      expect(
        filas,
        "un coordinador leyó filas del log de auditoría",
  ...
  test(
    ...
    async ({ coordinador }) => {
      ...
      await expect(
        coordinador.getByRole("link", { name: "Sedes" }),
      ).toBeVisible();
      await expect(
        coordinador.getByRole("link", { name: "Configuración" }),
      ).toBeVisible();
    },
  );
  ...
  test("la empresa B le dice que no es suya", async ({ coordinador }) => {
    ...
    await expect(coordinador.locator("body")).toContainText(
      /Sin acceso|no está asignado/i,
    );
  });
});
...
test.describe("Asesor", () => {
  test(
    ...
    async ({ asesorA }) => {
      ...
      await expect(
        asesorA.getByRole("link", { name: "Auditoría" }),
      ).toHaveCount(0);
      await expect(asesorA.getByRole("link", { name: "Sedes" })).toHaveCount(0);
      await expect(
        asesorA.getByRole("link", { name: "Configuración" }),
      ).toHaveCount(0);
      await expect(
        asesorA.getByRole("link", { name: /Nueva empresa/ }),
      ).toHaveCount(0);
    },
  );
  ...
  test(
    ...
    anotar({
      ...
      tipo: "feature",
      porque:
        "Quitarle de más le impediría trabajar, que es el otro modo de fallar.",
    }),
    async ({ asesorA }) => {
      await irA(asesorA, `/e/${EMPRESA_A}`);
  ...
  test("la empresa B le dice que no es suya", async ({ asesorA }) => {
    ...
    await expect(asesorA.locator("body")).toContainText(
      /Sin acceso|no está asignado/i,
    );
  });
  ...
  test(
    ...
    anotar({
      ...
      tipo: "seguridad",
      porque:
        "La lista de inicio no puede filtrar el nombre de otros clientes.",
    }),
    async ({ asesorA }) => {
  ...
  test(
    ...
    async ({ asesorA }) => {
      ...
      await expect(
        asesorA.getByRole("button", { name: /Invitar|Nuevo usuario/i }),
      ).toHaveCount(0);
    },
  );
});
