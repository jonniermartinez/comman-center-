...
function enlacesDe(texto: string, html: string): string[] {
  ...
  return [...new Set(encontrados.map((e) => e.replace(/&amp;/g, "&").replace(/[.,]+$/, "")))]
}

function datosDelEnlace(enlaces: string[]): { tokenHash: string | null; tipo: string | null } {
  for (const enlace of enlaces) {
    try {
      const url = new URL(enlace)
      const token = url.searchParams.get("token_hash") ?? url.searchParams.get("token")
      if (token) return { tokenHash: token, tipo: url.searchParams.get("type") }
    } catch {
...
export default {
  async email(message: ForwardableEmailMessage, env: Entorno) {
    ...
    const previos: Mensaje[] = JSON.parse((await env.CORREOS.get(clave)) ?? "[]")
    previos.unshift(mensaje)

    await env.CORREOS.put(clave, JSON.stringify(previos.slice(0, MAX_POR_BUZON)), {
      expirationTtl: TTL_SEGUNDOS,
    })
    console.log(`Correo guardado para ${para}: "${mensaje.asunto}" (${enlaces.length} enlaces)`)
  },
