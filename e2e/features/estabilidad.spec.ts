...
test.describe("Estabilidad del despliegue", () => {
  test("las páginas públicas responden sin agotar el worker", async ({ request }) => {
    const fallos: number[] = []

    for (let i = 0; i < PETICIONES; i++) {
      const respuesta = await request.get(`${BASE_URL}/login`, { failOnStatusCode: false })
  ...
  test("la pantalla de empresas aguanta una navegación normal", async ({ superAdmin }) => {
    const fallos: string[] = []

    // Ir y volver, como haría alguien revisando varias empresas.
    for (let i = 0; i < 5; i++) {
      const respuesta = await superAdmin.goto("/empresas", { waitUntil: "domcontentloaded" })
