...
test.describe("Captura diaria", () => {
  test(
    ...
    async ({ coordinador, apiSuperAdmin }) => {
      ...
      expect(
        venta,
        "la venta no llegó a la base pese a cerrarse el diálogo",
      ).toBeTruthy();
      expect(
        Number(venta!.valor_final),
        "el importe guardado no es el escrito",
      ).toBe(1500000);
...
test.describe("Edición de lo ya registrado", () => {
  test(
    ...
    async ({ coordinador, apiSuperAdmin }) => {
      ...
      expect(
        todas ?? [],
        "editar duplicó la venta en vez de cambiarla",
      ).toHaveLength(1);
      expect(Number(todas![0].valor_final), "el cambio no se guardó").toBe(
        1750000,
      );
