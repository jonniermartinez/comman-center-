...
test.describe("Alta de empresa", () => {
  test("un doble clic crea una sola empresa", async ({ superAdmin, apiSuperAdmin }) => {
  ...
  test("el botón dice Creando… mientras trabaja", async ({ superAdmin, apiSuperAdmin }) => {
  ...
  test("no deja crear dos empresas con el mismo nombre", async ({
    superAdmin,
    apiSuperAdmin,
  }) => {
...
test.describe("Borrado definitivo", () => {
  test("cuenta lo que se va a perder y exige el nombre exacto", async ({
    superAdmin,
    apiSuperAdmin,
  }) => {
    ...
    const borrar = superAdmin.getByRole("button", { name: /Eliminar para siempre/ })
    ...
    await expect
      .poll(async () => await empresaPorSlug(apiSuperAdmin, nombre), { timeout: 20_000 })
      .toBeNull()
  })
})
