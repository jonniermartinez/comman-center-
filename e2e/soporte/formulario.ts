...
export async function abrirDialogo(pagina: Page, boton: RegExp, titulo: RegExp) {
  await abrirDialogoDe(pagina, pagina.getByRole("button", { name: boton }).first(), titulo, boton)
}
...
export async function abrirDialogoDe(
  ...
  for (let intento = 1; intento <= 4; intento++) {
    ...
    try {
      await expect(dialogo.getByText(titulo).first()).toBeVisible({ timeout: 5_000 })
      return
    } catch {
      if (intento === 4) {
        throw new Error(
