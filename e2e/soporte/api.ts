...
async function abrirSesion(datos: Cuenta): Promise<Cliente> {
  ...
  throw new Error(
    `No se pudo iniciar sesión como ${datos.email}: ${ultimoError}`,
  );
}
...
export async function crearEmpresa(
  ...
  if (errorSede)
    throw new Error(`crearEmpresa/sede(${nombre}): ${errorSede.message}`);
  ...
  const { data: financiaciones } = await admin
...
export async function asignarAEmpresa(
  ...
  const empresa = await empresaPorSlug(admin, slug);
  if (!perfil || !empresa)
    throw new Error(`asignarAEmpresa: falta perfil ${email} o ${slug}`);
  ...
  if (error)
    throw new Error(`asignarAEmpresa(${email}, ${slug}): ${error.message}`);
}
 ...
 */
export async function vincularStaff(
  admin: Cliente,
  email: string,
  slug: string,
) {
  ...
  const empresa = await empresaPorSlug(admin, slug);
  if (!perfil || !empresa)
    throw new Error(`vincularStaff: falta perfil ${email} o ${slug}`);
